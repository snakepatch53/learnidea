...
TRATANDO DE EMULAR EL IMPORT DE NODE JS

<?php
function import(string $path, $functions_names) {
    ...
    $returns = [];
    foreach($functions_names as $index => $function) {
        // echo $index . " == " . $function . " ==> " . (is_numeric($index) ? "si" : "no");
        // echo "\n\n";
        ...
        // }
        $function_name = $function;
        // $use_function = is_numeric($index) ? $function : $index;
        $use_function = is_numeric($index) ? $function : $index;
        // echo $function_name . "\n\n";
        // echo is_numeric($index) == 1 ? "si" : "no" . "\n";
        if(!function_exists($use_function)) return die('No existe la funcion ' . $use_function);
        
        
         $tmp_fun = function(...$args) use ($use_function) {
            return call_user_func($use_function, ...$args);
        };
        
        $returns[$function_name] = fn (...$args) => $tmp_fun(...$args);
    }
    return $returns;
}


$functions = import('ruta.php', ['fun1' => 'otroName', 'fun2']);
